...
def generate_research_group():
    ...
    df_research_groups.to_sql(
        "ResearchGroup", con=engine, index=False, if_exists="replace"
    )
...
def generate_tutorial_user(tutorials, users):
    ...
    for i in range(number_of_tutorials):
        ...
        tutorial_users["tutorial"].append(
    ...
    df_tutorial_users.to_sql(
        "TutorialUser", con=engine, index=False, if_exists="replace"
    )
...
def generate_guide_project(guides, projects):
    ...
    for i in range(number_of_guides):
        ...
        guides_projects["project"].append(
    ...
    df_guides_projects.to_sql(
        "GuideProject", con=engine, index=False, if_exists="replace"
    )
...
def generate_project_user(users, projects):
    ...
    df_project_users.to_sql(
        "ProjectUsers", con=engine, index=False, if_exists="replace"
    )
...
def generate_project_post_comment(users, post):
    ...
    df_project_post.to_sql(
        "ProjectPostComment", con=engine, index=False, if_exists="replace"
    )
    return df_project_post
...
def generate_research_group_users(df_users, df_guides):
    ...
    for i in range(number_of_research_group_users):
        ...
        research_group_users["person"].append(
            df_users["id"].values[i % len(df_users["id"])]
        )
        research_group_users["research_group"].append(
    ...
    df_research_group_users.to_sql(
        "ResearchGroupUser", con=engine, index=False, if_exists="replace"
    )
...
def generate_research_group_guides(df_research_groups, df_guides):
    ...
    for i in range(number_of_research_group_guides):
        ...
        research_group_guides["research_group"].append(
            df_research_groups["id"].values[i % len(df_research_groups["id"])]
        )
        research_group_guides["guide"].append(
            df_guides["id"].values[i % len(df_guides["id"])]
        )
    ...
    df_research_group_guides.to_sql(
        "ResearchGroupGuides", con=engine, index=False, if_exists="replace"
    )
...
def generate_research_group_post(df_users, df_research_group):
    ...
    for i in range(number_of_research_group_posts):
        ...
        research_group_post["author"].append(
            df_users["id"].values[i % len(df_users["id"])]
        )
        ...
        research_group_post["research_group"].append(
    ...
    df_research_group_post.to_sql(
        "ResearchGroupPost", con=engine, index=False, if_exists="replace"
    )
...
def generate_research_group_post_comment(df_users, df_research_group_post):
    ...
    df_research_group_post_comment.to_sql(
        "ResearchGroupPostComment", con=engine, index=False, if_exists="replace"
    )
...
def generate_research_group_disk(df_projects, df_links):
    ...
    for i in range(number_of_research_group_disks):
        ...
        research_group_disk["research_group"].append(
            df_projects["id"].values[i % len(df_projects["id"])]
        )
        research_group_disk["link"].append(
            df_links["id"].values[i % len(df_links["id"])]
        )
    df_research_group_disk = pd.DataFrame(research_group_disk)
    if is_printing:
        print(df_research_group_disk)
    df_research_group_disk.to_sql(
        "ResearchGroupDisk", con=engine, index=False, if_exists="replace"
    )
...
def generate_research_group_link(df_projects, df_links):
    ...
    for i in range(number_of_research_group_links):
        ...
        research_group_link["research_group"].append(
            df_projects["id"].values[i % len(df_projects["id"])]
        )
        research_group_link["link"].append(
            df_links["id"].values[i % len(df_links["id"])]
        )
    ...
    df_research_group_link.to_sql(
        "ResearchGroupLink", con=engine, index=False, if_exists="replace"
    )
...
def generate_announcement(df_users, df_research_group):
    ...
    for i in range(number_of_announcements):
        ...
        announcements["research_group_id"].append(
            df_research_group["id"].values[i % len(df_research_group["id"])]
        )
        types = ["def", "Default"]
    ...
    df_announcements.to_sql(
        "Announcement", con=engine, index=False, if_exists="replace"
    )
